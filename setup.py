...
def get_cuda_arch_flags():
    return [
        ...
        '-gencode', 'arch=compute_120a,code=sm_120a',  # Geforce Blackwell
        '--expt-relaxed-constexpr',  # Fix for CUTE nullspace function
    ]
    ...
    setup(
        ...
        ext_modules=[
            CUDAExtension(
                name='serq._CUDA',
                sources=[
                    'serq/kernel/bindings.cpp',
                    'serq/kernel/gemm.cu',
                    'serq/kernel/mx_quant.cu'
                ],
                include_dirs=[
                    os.path.join(setup_dir, 'serq/kernel/include'),
